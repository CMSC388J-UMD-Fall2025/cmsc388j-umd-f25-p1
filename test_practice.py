...
def test_small_sum_multiples():
    ...
    assert 2 == p.sum_multiples(3)

    assert 6 == p.sum_multiples(5)
    assert 6 == p.sum_multiples(6)
    assert 12 == p.sum_multiples(7)


def test_large_sum_multiples():
    assert 63 == p.sum_multiples(16)
    assert 184 == p.sum_multiples(25)
    assert 434280 == p.sum_multiples(1234)

def test_valid_twosum():
    assert [0, 1] == p.two_sum([2, 7, 11, 15], 9)
    assert [1, 2] == p.two_sum([3, 2, 4], 6)
    assert [0, 1] == p.two_sum([3, 3], 6)

def test_invalid_twosum():
    assert -1 == p.two_sum([1, 2, 3], 7)
